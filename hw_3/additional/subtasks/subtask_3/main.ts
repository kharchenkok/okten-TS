...
let randomRangeNumberArr:number[]=[];
for(let i = 0; i<20; i+=1){
...
let pricesArr:number[] = [100,250,50,168,120,345,188];
...
console.log('============Task 8==============');
const mixArr1:any[] = [false,['Hello','World'],true,123,'Okten',{name:'Apple'},{name:'Banana'},0,'Orange','Grape',3.14]
const newNumberArr:number[]=[];
for(const elem of mixArr1){
...
interface CityWithId {
    user_id: number;
    country: string;
    city: string;
}
interface UserWithCity extends UserWithId{
     address: CityWithId;
}
let usersWithId:UserWithId[] = [
...
let usersWithCities:UserWithCity[] = [];
...
word="";
let counter1=0;
while (counter1<lettersArr.length){
    word+=lettersArr[counter1];
    counter1+=1;
}
console.log('word:', word);
